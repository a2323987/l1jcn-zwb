	...
	public static void loadTable(String lang) throws IOException {
		...
		try {
			...
			while ((line = inr.readLine()) != null) {
				...
				if (!line.isEmpty()
//						&& line.getBytes().length > 4 // 短い文字列("肉"など)は置换するとかえって长くなることが
		...
		for (Object o : _desc) {
			...
			if (replacement.indexOf(d.getText()) > -1) {
				...
				if (replacement.matches(ptn1) || replacement.matches(ptn2)) {
					// 单语单位でのみ置换する(前后に空白がないような场合は置换しない)
					continue;
				}
...
// 置换方法を变えたので不要に
/**
 * 引数の文字列のうちdesc-j.tblに存在する部分を$xxxxの形に置换した文字列を返す。<br>
 * 例: "强化 グリーン ポーション" -> "$1652 $234"<br>
 * 例: "オアシス テレポート スクロール" -> "オアシス $230"<br>
 * 
 * @param str
 *            文字列
 * @param takeShorter
 *            true: 置换前の方がバイト数が少なくなる场合置换しない
 * @return 置换された文字列
 */
//public static String getReplacedString(String str, boolean takeShorter) {
//	if (str.isEmpty()) {
