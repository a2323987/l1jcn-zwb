...
public class S_Paralysis extends ServerBasePacket {
	...
	public S_Paralysis(int type, boolean flag) {
		...
		if (type == TYPE_PARALYSIS) // 体が完全に麻痹しました。
		{
			if (flag == true) {
				writeC(2);
			} else {
		...
		} else if (type == TYPE_PARALYSIS2) // 体が完全に麻痹しました。
		{
			if (flag == true) {
				writeC(4);
			} else {
		...
		} else if (type == TYPE_TELEPORT_UNLOCK) // テレポート待ち状态の解除
		{
			writeC(7);
		} else if (type == TYPE_SLEEP) // 强力な睡魔が袭ってきて、寝てしまいました。
		{
			if (flag == true) {
				writeC(10);
			} else {
		...
		} else if (type == TYPE_FREEZE) // 体が冻りました。
		{
			if (flag == true) {
				writeC(12);
			} else {
		...
		} else if (type == TYPE_STUN) // スタン状态です。
		{
			if (flag == true) {
				writeC(22);
			} else {
		...
		} else if (type == TYPE_BIND) // 足が缚られたように动けません。
		{
			if (flag == true) {
				writeC(24);
			} else {
