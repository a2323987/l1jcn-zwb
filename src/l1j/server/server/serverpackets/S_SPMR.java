...
public class S_SPMR extends ServerBasePacket {
	...
	private void buildPacket(L1PcInstance pc) {
		...
		// ウィズダムポーションのSPはS_SkillBrave送信时に更新されるため差し引いておく
		if (pc.hasSkillEffect(STATUS_WISDOM_POTION)) {
			writeC(pc.getSp() - pc.getTrueSp() - 2); // 装备增加したSP
		}
		else {
			writeC(pc.getSp() - pc.getTrueSp()); // 装备增加したSP
		}
		writeH(pc.getTrueMr() - pc.getBaseMr()); // 装备や魔法で增加したMR
	}
