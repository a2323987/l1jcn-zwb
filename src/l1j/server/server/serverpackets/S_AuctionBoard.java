...
public class S_AuctionBoard extends ServerBasePacket {
	...
	private void buildPacket(L1NpcInstance board) {
		...
		try {
			...
			while (rs.next()) {
				houseId = rs.getInt(1);
				if ((board.getX() == 33421) && (board.getY() == 32823)) { // 竞卖揭示板(ギラン)
					if ((houseId >= 262145) && (houseId <= 262189)) {
						houseList.add(houseId);
						count++;
					}
				}
				else if ((board.getX() == 33585) && (board.getY() == 33235)) { // 竞卖揭示板(ハイネ)
					if ((houseId >= 327681) && (houseId <= 327691)) {
						houseList.add(houseId);
						count++;
					}
				}
				else if ((board.getX() == 33959) && (board.getY() == 33253)) { // 竞卖揭示板(アデン)
					if ((houseId >= 458753) && (houseId <= 458819)) {
						houseList.add(houseId);
						count++;
					}
				}
				else if ((board.getX() == 32611) && (board.getY() == 32775)) { // 竞卖揭示板(グルーディン)
		...
		for (int i = 0; i < count; ++i) {
			...
			writeH(area[i]); // アジトの广さ
			writeC(month[i]); // 缔切月
			writeC(day[i]); // 缔切日
			writeD(price[i]); // 现在の入札価格
		}
	}
