...
public class FaceToFace {
	...
	public static L1PcInstance faceToFace(L1PcInstance pc) {
		...
		if (players.size() == 0) { // 1セル以内にPCが居ない场合
			pc.sendPackets(new S_ServerMessage(93)); // \f1そこには谁もいません。
			return null;
		}
		for (L1PcInstance target : players) {
			...
			if ((pcHeading == 0) && (pcX == targetX) && (pcY == (targetY + 1))) {
				if (targetHeading == 4) {
					...
					pc.sendPackets(new S_ServerMessage(91, target.getName())); // \f1%0があなたを见ていません。
					return null;
				}
			}
