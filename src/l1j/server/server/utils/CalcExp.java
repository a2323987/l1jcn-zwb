...
import l1j.server.server.templates.L1MagicDoll; // 经验加倍魔法娃娃
...
public class CalcExp {
	...
	private static void AddExp(L1PcInstance pc, int exp, int lawful) {
		...
		int _nLevel_t = pc.getLevel(); // 经验值回馈奖励系统
		...
		// end
		// 殷海萨加成条件
		double _nExpRate = exppenalty * foodBonus * expBonus * LevelBonus * L1MagicDoll.getDoubleExpByDoll(pc); // 经验加倍魔法娃娃
		...
		if (pc.getAccessLevel()== 200 || pc.getAccessLevel()== 100) {
			pc.sendPackets(new S_ServerMessage(166,"总共取得 " + add_exp +" 经验值")); //\f1%0%s %4%1%3 %2。
		}
		// end
	}
	...
	private static void AddExpPet(L1PetInstance pet, int exp) {
		...
		// end
		int totalExp = (int) (exp * exp_wa * Config.RATE_XP_PET + pet.getExp()); // 宠物经验倍率
