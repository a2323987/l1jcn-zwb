...
public class C_Attr extends ClientBasePacket {
	...
	public C_Attr(byte abyte0[], ClientThread clientthread) throws Exception {
		...
		case 322: // 是否要复活？ (Y/N)
			...
			pc.setTempID(0);
			if (resusepc2 != null) { // 祝福された 复活スクロール、リザレクション、グレーター リザレクション
				if (c == 0) { // No
		...
		case 630: // %0%s 要与你决斗。你是否同意？(Y/N)
			...
			if (c == 0) {
				...
				fightPc.sendPackets(new S_ServerMessage(631, pc.getName())); // %0%dがあなたとの决斗を断りました。
			} else if (c == 1) {
		...
		case 654: // %0 向你(你)求婚，你(你)答应吗?
			...
			if (partner != null) {
				if (c == 0) { // No
					...
					pc.save();
					pc.sendPackets(new S_ServerMessage( // 俩人的结婚在所有人的祝福下完成
							790));
