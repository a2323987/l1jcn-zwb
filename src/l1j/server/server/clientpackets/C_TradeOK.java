	...
	public C_TradeOK(byte abyte0[], ClientThread clientthread) throws Exception {
		...
		if (trading_partner != null) {
			...
			if (player.getTradeOk() && trading_partner.getTradeOk()) // 同时都压OK
			{
				...
				{
					player.sendPackets(new S_ServerMessage(263)); // \f1一人のキャラクターが持って步けるアイテムは最大180个までです。
					trading_partner.sendPackets(new S_ServerMessage(263)); // \f1一人のキャラクターが持って步けるアイテムは最大180个までです。
					L1Trade trade = new L1Trade();
