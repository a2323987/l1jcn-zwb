			...
			if (obj instanceof L1NpcInstance) {
				...
				// 掉落物品过滤系统
					...
					if ((awakeSkillId == AWAKEN_ANTHARAS)
						...
						target.sendPackets(new S_ServerMessage(1384)); // 现在の状态では变身できません。
						return;
					}
...
//		sosodemon add william end
				...
				if (pay < 1) {
					...
					if ((item != null)
						...
						pc.sendPackets(new S_ServerMessage(166,
								"所持有的金币超过2,000,000,000")); //修正讯息多一个句号
						htmlid = "";
					} else if ((item != null)
		...
		} else if (s.equalsIgnoreCase("aggressive")) { // 攻击态势
			if (obj instanceof L1PetInstance) {
		...
		// アレックス(歌う岛)
			...
			if (s.equalsIgnoreCase("1")) {
				...
				L1ItemInstance item = pc.getInventory().storeItem(41227, 1); // アレックスの绍介状
			...
			if (s.equalsIgnoreCase("A")) {
				...
				if (pc.getInventory().consumeItem(41344, 1)) { // 水の精粹
					L1ItemInstance item = ItemTable.getInstance().createItem(
							21058); // 训练骑士のマント2
					if (item != null) {
						pc.getInventory().consumeItem(21057, 1); // 训练骑士のマント1
						if (pc.getInventory().checkAddItem(item, 1) == 0) {
							pc.getInventory().storeItem(item);
							pc.sendPackets(new S_ServerMessage(143,
									((L1NpcInstance) obj).getNpcTemplate()
				...
				if (pc.getInventory().consumeItem(41345, 1)) { // 酸性の乳液
					L1ItemInstance item = ItemTable.getInstance().createItem(
							21059); // ポイズン
									...
									// クローク
					if (item != null) {
						pc.getInventory().consumeItem(21058, 1); // 训练骑士のマント2
						if (pc.getInventory().checkAddItem(item, 1) == 0) {
							pc.getInventory().storeItem(item);
							pc.sendPackets(new S_ServerMessage(143,
									((L1NpcInstance) obj).getNpcTemplate()
		...
		// ジェロン
			...
			if (s.equalsIgnoreCase("A")) {
				...
				if (pc.getInventory().checkItem(41340, 1)) { // 佣兵团长 ティオンの绍介状
		...
		// 谍报员(欲望の洞窟侧)
			...
			if (s.equalsIgnoreCase("n")) {
				...
				htmlid = "";
				L1ItemInstance item = pc.getInventory().storeItem(41130, 1); // 血痕の契约书
				pc.sendPackets(new S_ServerMessage(143, ((L1NpcInstance) obj)
						.getNpcTemplate().get_name(), item.getItem().getName()));
			}
		}
		// 谍报员(影の神殿侧)
			...
			if (s.equalsIgnoreCase("n")) {
				...
				htmlid = "";
				L1ItemInstance item = pc.getInventory().storeItem(41121, 1); // カヘルの契约书
				pc.sendPackets(new S_ServerMessage(143, ((L1NpcInstance) obj)
						.getNpcTemplate().get_name(), item.getItem().getName()));
			}
		}
		...
		// エルラス
			...
			if (s.equalsIgnoreCase("0")) {
				...
				if (pc.getLevel() < 30) {
					...
					createitem = new int[] { 49149 }; // シャルナの变身スクロール（レベル30）
					createcount = new int[] { 1 };
				} else if ((pc.getLevel() >= 40) && (pc.getLevel() <= 51)) {
					createitem = new int[] { 49150 }; // シャルナの变身スクロール（レベル40）
					createcount = new int[] { 1 };
				} else if ((pc.getLevel() >= 52) && (pc.getLevel() <= 54)) {
					createitem = new int[] { 49151 }; // シャルナの变身スクロール（レベル52）
					createcount = new int[] { 1 };
				} else if ((pc.getLevel() >= 55) && (pc.getLevel() <= 59)) {
					createitem = new int[] { 49152 }; // シャルナの变身スクロール（レベル55）
					createcount = new int[] { 1 };
				} else if ((pc.getLevel() >= 60) && (pc.getLevel() <= 64)) {
					createitem = new int[] { 49153 }; // シャルナの变身スクロール（レベル60）
					createcount = new int[] { 1 };
				} else if ((pc.getLevel() >= 65) && (pc.getLevel() <= 69)) {
					createitem = new int[] { 49154 }; // シャルナの变身スクロール（レベル65）
					createcount = new int[] { 1 };
				} else if (pc.getLevel() >= 70) {
					createitem = new int[] { 49155 }; // シャルナの变身スクロール（レベル70）
					createcount = new int[] { 1 };
				}
		...
		// 幻术士 试练任务
		...
		} else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 81334) { // 被遗弃的肉身
			if (s.equalsIgnoreCase("a")) {
				...
				&& (((L1NpcInstance) obj).getNpcTemplate().get_npcId() <= 81363)) { // 魔法商人- 仿正设定   
			...
			if (s.equalsIgnoreCase("a") || s.equalsIgnoreCase("b")){
				if(pc.getInventory().consumeItem(L1ItemId.ADENA,Integer.valueOf(l1j.william.L1WilliamSystemMessage.ShowMessage(127)).intValue())){
					pc.setCurrentHp(pc.getMaxHp());
					pc.setCurrentMp(pc.getMaxMp());
					L1SkillUse l1skilluse = new L1SkillUse();
		...
		// 多鲁嘉贝尔
			...
			if (s.equalsIgnoreCase("0")) {
				if (pc.getInventory().checkItem(46000, 1)) { // 检查身上是否有多鲁嘉之袋
					htmlid = "veil3"; // 已经有袋子了
				} else if (pc.getInventory().checkItem(L1ItemId.ADENA, 1000000)) { // 检查身上金币是否足够
	...
	private boolean usePolyScroll(L1PcInstance pc, int itemId, String s) {
		int time = 0;
		if ((itemId == 40088) || (itemId == 40096)) { // 变身スクロール、象牙の塔の变身スクロール
			time = 1800;
		} else if (itemId == 140088) { // 祝福された变身スクロール
			time = 2100;
		}
		...
		if (isUseItem) {
			...
			pc.sendPackets(new S_ServerMessage(181)); // \f1そのようなモンスターには变身できません。
		}
		return isUseItem;
	}
