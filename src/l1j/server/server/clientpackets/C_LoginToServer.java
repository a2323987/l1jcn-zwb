...
import static l1j.server.server.model.skill.L1SkillId.LOGIN_TIME_GET_ITEM; // 在线一段时间后给予一个状态
import static l1j.server.server.model.skill.L1SkillId.LOGIN_EIN_TIME; // 殷海萨的祝福
...
import java.util.Collection; // 玩家上线是否通知在线ＧＭ 4/5
import java.util.Calendar; // 殷海萨的祝福
	...
	public C_LoginToServer(byte abyte0[], ClientThread client)
		...
		if (Config.LEVEL_DOWN_RANGE != 0) {
			if (pc.getHighLevel() - pc.getLevel() >= Config.LEVEL_DOWN_RANGE) {
				_log.info("登录请求超出了容忍的等级下降的角色: char=" + charName + " account="
		...
		//add GUI by Eric
		if(Config.GUI)
			l1j.gui.Eric_J_Main.getInstance().addPlayerTable(login,
					...
					for (L1PcInstance clanMember : clanMembers) {
						if (clanMember.getId() != pc.getId()) {
							clanMember.sendPackets(new S_ServerMessage(843, pc
									.getName())); // 只今、血盟员の%0%sがゲームに接续しました。
						}
					}
		...
		if (Config.EINHASAD_IS_ACTIVE) {
			...
			if (pc.isEinLevel()) { // 49级以上 殷海萨的祝福
				if (pc.isEinZone()) { // 安全区域
					int einOutTime = Config.EIN_OUTTIME;
		...
		if (Config.WHO_ONLINE_MSG_ON) {
			...
			for (L1PcInstance object : allGM)
				if (object instanceof L1PcInstance) {
					...
					if (pc.getClanid() >= 0
							&& (GM.getAccessLevel() == 200 || GM
									.getAccessLevel() == 100)) {
						GM.sendPackets(new S_ServerMessage(166, ("玩家 ")
								+ pc.getName() + ("[")
								+ client.getAccountName() + ("] 上线 (")
								+ client.getIp() + (")")));
					}
				}
		}
