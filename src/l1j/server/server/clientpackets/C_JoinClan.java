...
public class C_JoinClan extends ClientBasePacket {
	...
	private void JoinClan(L1PcInstance player, L1PcInstance target) {
		...
		if (clan_id == 0) { // 面对的对象没有创立血盟
			player.sendPackets(new S_ServerMessage(90, target.getName())); // \f1%0は血盟を创设していない状态です。
			return;
		}
		...
		if (player.getClanid() != 0) { // 已经加入血盟
			if (player.isCrown()) { // 自己是盟主
				...
				if ((player_clan.getCastleId() != 0) || // 有城堡或有血盟小屋
					...
					player.sendPackets(new S_ServerMessage(665)); // \f1城やアジトを所有した状态で血盟を解散することはできません。
					return;
				}
			}
			else {
		...
		target.sendPackets(new S_Message_YN(97, player.getName())); // %0が血盟に加入したがっています。承诺しますか？（Y/N）
	}
