...
import l1j.server.server.serverpackets.S_SkillSound; // 您收到鸽子信差给你的信件 by tjohny
		...
		if ((type == 0x00) || (type == 0x01) || (type == 0x02)) { // 开启
			...
			if (receiver != null) { // 对方在线上
				...
				if (receiver.getOnlineStatus() == 1) {
					...
					// 您收到鸽子信差给你的信件 by tjohny
					receiver.sendPackets(new S_SkillSound(receiver.getId(), 1091));
					receiver.sendPackets(new S_ServerMessage(428));
					// end
				}
			}
			else { // 对方离线中
