...
import l1j.server.server.model.item.action.FilterItem; // 掉落物品过滤系统 by missu0524
...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		case 300005: //武器敏捷捲軸 
		case 300006://武器智力捲軸
		case 300007://魔法攻击力武器卷轴
		case 300008://防冻结耐性斗篷卷轴
		case 300009://防石化耐性盔甲卷轴
		case 300010://防睡眠耐性T恤卷轴
		case 300011://防黑暗耐性长靴卷轴
		...
		if (pc.getCurrentHp() > 0) {
			...
			if ((itemId == 40077)
				...
				Enchant.scrollOfEnchantWeaponIvoryTower(pc, l1iteminstance,
					...
					|| (itemId == 41431) || (itemId == 41432)) { // 武器属性强化卷轴
				Enchant.scrollOfEnchantWeaponAttr(pc, l1iteminstance,
						l1iteminstance1, client);
...
//				sosodemon add 物品声望控制
				...
				if (l1iteminstance.getItem().getType() == 0) { // アロー
				...
				} else if (l1iteminstance.getItem().getType() == 18) { // 道具传送卷轴(符)类
					if (L1WilliamTeleportScroll.checkItemId(itemId) != 0) {
						L1WilliamTeleportScroll.getTeleportScroll(pc,
								l1iteminstance, itemId);
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				} else if (l1iteminstance.getItem().getType() == 19) { // 道具融合系统类
					if (L1Blend.checkItemId(itemId) != 0) {
						L1Blend.getItemBlend(pc, l1iteminstance, itemId);
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				} else if (l1iteminstance.getItem().getType() == 20) { // 家具类
					if (itemId == 41401) { // 移除家具魔杖
				...
				} else if (itemId == 43000) { // 复活のポーション（Lv99キャラのみが使用可能/Lv1に戻る效果）
					pc.setExp(1);
					pc.resetLevel();
					pc.setBonusStats(0);
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
					pc.sendPackets(new S_ServerMessage(822)); // 独自アイテムですので、メッセージは适当です。
				...
				// 治愈药水、浓缩体力恢复剂、象牙塔治愈药水
					...
					Potion.UseHeallingPotion(pc, l1iteminstance, 70, 197);
						...
						|| (itemId == 40028)) { // 香蕉汁、橘子汁、苹果汁
					Potion.UseHeallingPotion(pc, l1iteminstance, 25, 189);
				} else if ((itemId == 40058) || (itemId == 49268)
						|| (itemId == 49269)) { // 烟熏的面包屑、爱玛伊的画像、伊森之画像
				...
				} else if (itemId == L1ItemId.B_POTION_OF_HEALING) { // 受祝福的
																		// 治愈药水
					Potion.UseHeallingPotion(pc, l1iteminstance, 25, 189);
				} else if (itemId == L1ItemId.C_POTION_OF_HEALING) { // 受咀咒的
																		// 治愈药水
					Potion.UseHeallingPotion(pc, l1iteminstance, 10, 189);
				} else if (itemId == L1ItemId.B_POTION_OF_EXTRA_HEALING) { // 受祝福的
																			// 强力治愈药水
					Potion.UseHeallingPotion(pc, l1iteminstance, 55, 194);
				} else if (itemId == L1ItemId.B_POTION_OF_GREATER_HEALING) { // 受祝福的
																				// 终极治愈药水
				...
				} else if ((itemId >= 41417) && (itemId <= 41421)) { // 日本“亚丁的夏天”活动道具
																		// - 刨冰
					...
					Potion.ThirdSpeed(pc, l1iteminstance, 600);
						...
						&& (itemId <= L1ItemId.SCROLL_FOR_ENCHANTING_BATTLE)) { // 150%神力药水
																				// ~
																				// 强化战斗卷轴
					...
					Potion.useBluePotion(pc, l1iteminstance, itemId);
						...
						|| (itemId == L1ItemId.B_POTION_OF_EMOTION_WISDOM) // 受祝福的
																			// 慎重药水
						|| (itemId == 49307)) { // 福利慎重药水
					if (pc.isWizard()) { // 法师
					...
					if ((l1iteminstance1.getItem().getType2() != 0)
						...
						if (l1iteminstance1.get_durability() == 0) {
							pc.sendPackets(new S_ServerMessage(464, msg0)); // %0%sは新品同样の状态になりました。
						} else {
							pc.sendPackets(new S_ServerMessage(463, msg0)); // %0の状态が良くなりました。
						}
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
					...
					if (l1iteminstance1.getItemId() >= 47053
							&& l1iteminstance1.getItemId() <= 47062) { // 附魔石 ~
																		// 9阶附魔石
						int type = (itemId - 47048) * 10; // type = 10,20,30,40
						int rnd = Random.nextInt(100) + 1;
						if (Config.MAGIC_STONE_TYPE < rnd) {
							int newItem = l1iteminstance1.getItemId() + type; // 附魔石(近战)
																				// ~
																				// 9阶附魔石(近战)
							...
							pc.sendPackets(new S_ServerMessage(1411,
									l1iteminstance1.getName())); // 对\f1%0附加魔法失败。
						}
						pc.getInventory().removeItem(l1iteminstance1, 1); // 删除
																			// -
																			// 附魔石
						pc.getInventory().removeItem(l1iteminstance, 1); // 删除 -
																			// 附魔转换卷轴
					} else {
						pc.sendPackets(new S_ServerMessage(79));
					}
					...
					if (Config.MAGIC_STONE_LEVEL < rnd
						...
						int newItem = l1iteminstance1.getItemId() + 1; // X 阶附魔石
																		// ->
																		// X+1
																		// 阶附魔石
						...
						pc.sendPackets(new S_ServerMessage(1410,
								l1iteminstance1.getName())); // 对\f1%0附加强大的魔法力量成功。
				...
				} else if ((itemId == 47068) || (itemId == 47069)
						|| (itemId == 47078) || (itemId == 47079)
						|| (itemId == 47088) || (itemId == 47089)
						|| (itemId == 47098) || (itemId == 47099)) { // 5 ~
																		// 6阶附魔石(近战)(远攻)(恢复)(防御)
				...
				} else if ((itemId == 47070) || (itemId == 47080)
						|| (itemId == 47090) || (itemId == 47100)) { // 7阶附魔石(近战)(远攻)(恢复)(防御)
				...
				} else if ((itemId == 47071) || (itemId == 47081)
						|| (itemId == 47091) || (itemId == 47101)) { // 8阶附魔石(近战)(远攻)(恢复)(防御)
				...
				} else if ((itemId == 47072) || (itemId == 47082)
						|| (itemId == 47092) || (itemId == 47102)) { // 9阶附魔石(近战)(远攻)(恢复)(防御)
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
					pc.sendPackets(new S_ServerMessage(155)); // \f1谁かが助けてくれたようです。
				} else if ((itemId == 40126) || (itemId == 40098)) { // 确认スクロール
					...
					// 新增可从DB自行输入鉴定描述 by 13630020
					String name = l1iteminstance1.getItem().getName();
					String desc = l1iteminstance1.getItem().getItemDescExtra();
					if ((l1iteminstance1.getItem().getItemDescId() == 0)
					...
					if ((earing3Id >= 41161) && (41172 >= earing3Id)) {
						...
						if (earing3Id == (itemId + 230)) {
							if ((Random.nextInt(99) + 1) < Config.CREATE_CHANCE_PROCESSING) {
								int[] earingId = { 41161, 41162, 41163, 41164,
										41165, 41166, 41167, 41168, 41169,
										41170, 41171, 41172 };
								int[] earinglevel = { 21014, 21006, 21007,
										21015, 21009, 21008, 21016, 21012,
										21010, 21017, 21013, 21011 };
								...
								pc.sendPackets(new S_ServerMessage(158,
										l1iteminstance1.getName())); // \f1%0%s
																		// 消失。
							}
							pc.getInventory().removeItem(l1iteminstance1, 1);
							pc.getInventory().removeItem(l1iteminstance, 1);
					...
					if (pc.isWizard()) { // ウィザード
						if (((itemId == 40090) && (blanksc_skillid <= 7)) || // ブランク
							...
							pc.sendPackets(new S_ServerMessage(591)); // \f1スクロールがそんな强い魔法を记录するにはあまりに弱いです。
						}
					} else {
						pc.sendPackets(new S_ServerMessage(264)); // \f1你的职业无法使用此道具。
					}
					...
					// スペルスクロール
						...
						|| ((itemId >= 49281) && (itemId <= 49286))) { // 40863はテレポートスクロールとして处理される
					if ((spellsc_objid == pc.getId())
							&& (l1iteminstance.getItem().getUseType() != 30)) { // spell_buff
						pc.sendPackets(new S_ServerMessage(281)); // \f1魔法が无效になりました。
						return;
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
					if ((spellsc_objid == 0)
					...
					pc.sendPackets(new S_UseMap(pc, l1iteminstance.getId(),
						...
						|| (itemId == 40731) || (itemId == 40732)) { // 便笺(未使用)
					...
					pc.getInventory().saveItem(l1iteminstance,
						...
						|| (itemId == 49025)) { // 便笺(开封济み)
					pc.sendPackets(new S_Letter(l1iteminstance));
				} else if ((itemId == 40314) || (itemId == 40316)) { // ペットのアミュレット
					if (pc.getInventory().checkItem(41160)) { // 召唤の笛
				...
				} else if ((itemId == 40089) || (itemId == 140089)) { // 复活スクロール、祝福された复活スクロール
							...
							if (L1World.getInstance()
								...
								for (L1PcInstance visiblePc : L1World
										.getInstance().getVisiblePlayer(target,
												0)) {
									if (!visiblePc.isDead()) {
										// \f1その场所に他の人が立っているので复活させることができません。
				...
				} else if ((itemId >= 49102) && (itemId <= 49116)) { // ドラゴンナイトの书板
				...
				} else if ((itemId >= 49117) && (itemId <= 49136)) { // 记忆の水晶
					if (pc.isIllusionist() || pc.isGm()) {
						...
						|| (itemId == 40521)) { // 传送回家的卷轴、象牙塔传送回家的卷轴、精灵羽翼
				...
				} else if (itemId == 40124) { // 血盟归还スクロール
					if (pc.getMap().isEscapable() || pc.isGm()) {
						int castle_id = 0;
						int house_id = 0;
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if ((itemId >= 40901) && (itemId <= 40908)) { // 各种エンゲージリング
				...
				} else if (itemId == 49166) { // 圣殿3楼钥匙
					if (pc.getQuest().get_step(L1Quest.QUEST_LEVEL50) > 0) {
						if (pc.isCrown() && (pc.getMapId() == 2000)) {
							short mapid = 2000;
							L1Teleport.teleport(pc, 32735, 32748, mapid, 5,
									true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isKnight() && (pc.getMapId() == 2001)) {
							short mapid = 2001;
				...
				} else if (itemId == 49239) { // 消灭之意志
					if (pc.getQuest().get_step(L1Quest.QUEST_LEVEL50) > 0) {
						if (pc.isCrown() && (pc.getMapId() == 2000)) {
							short mapid = 2000;
							L1Teleport.teleport(pc, 32793, 32757, mapid, 5,
									true);
							pc.getInventory().removeItem(l1iteminstance, 1);
						} else if (pc.isKnight() && (pc.getMapId() == 2001)) {
							short mapid = 2001;
					...
					if (pc.isElf()
							...
							&& (pc.getMapId() == 4)
							&& !pc.getInventory().checkItem(40548)) { // 亡灵の袋
						boolean found = false;
						...
						if (found) {
							...
							L1SpawnUtil.spawn(pc, 45300, 0, 0); // 古代人の亡灵
						}
					} else {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
					}
				} else if (itemId == 40557) { // 暗杀リスト(グルーディン)
				...
				} else if (itemId == 40563) { // 暗杀リスト(火田村)
				...
				} else if (itemId == 40561) { // 暗杀リスト(ケント)
				...
				} else if (itemId == 40560) { // 暗杀リスト(ウッドベック)
				...
				} else if (itemId == 40562) { // 暗杀リスト(ハイネ)
				...
				} else if (itemId == 40559) { // 暗杀リスト(アデン)
				...
				} else if (itemId == 40558) { // 暗杀リスト(ギラン)
					...
					if (pc.getMap().isUsePainwand()) {
						...
						if ((itemId == 40006) || (itemId == 140006)) {
							...
							if (l1iteminstance.getChargeCount() <= 0) { // 次数为
																		// 0时删除
								pc.getInventory().removeItem(l1iteminstance, 1);
							}
						} else {
					...
					data = new int[] { ActionCodes.ACTION_Wand, dmg, 10, 6 }; // data
																				// =
																				// {actid,
																				// dmg,
																				// spellgfx,
			...
			//todo 
				} else if (itemId == 60020) // 卡点自救
				{
					...
					if (((pc.getX() >= 32701) && (pc.getX() <= 32705))
						...
						|| (itemId == 40783)) { // 暗影神殿3楼钥匙
					if (((pc.getX() >= 32698) && (pc.getX() <= 32702))
							&& ((pc.getY() >= 32894) && (pc.getY() <= 32898))
							&& (pc.getMapId() == 523)) { // 影の神殿2阶
						L1Teleport.teleport(pc, ((L1EtcItem) l1iteminstance
								.getItem()).get_locx(),
								((L1EtcItem) l1iteminstance.getItem())
										.get_locy(),
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 41316) { // 神圣なミスリル パウダー
					if (pc.hasSkillEffect(STATUS_HOLY_WATER_OF_EVA)) {
						pc.sendPackets(new S_ServerMessage(79)); // \f1没有任何事情发生。
						return;
					}
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 49168) { // 破坏之密药
					pc.setSkillEffect(SECRET_MEDICINE_OF_DESTRUCTION,
							900 * 1000);
					pc.sendPackets(new S_SkillSound(pc.getId(), 190));
					pc.broadcastPacket(new S_SkillSound(pc.getId(), 190));
					pc.getInventory().removeItem(l1iteminstance, 1);
					...
					Enchant.scrollOfEnchantAccessory(pc, l1iteminstance,
							l1iteminstance1, client);
				...
				// TODO 自订道具区
					...
					if (check_item != null && check_item.getItem().getType2() == 1) { // 武器類
						if (Random.nextInt(100) > 50 && l1iteminstance1.getUpdateDmg() <= 10) {
							...
							if(l1iteminstance1.getUpdateCount() > 0){
							l1iteminstance1.setUpdateCount(l1iteminstance1.getUpdateCount() - 1);
							}
					...
					if (check_item != null && check_item.getItem().getType2() == 1) { // 武器類
						if (Random.nextInt(100) > 50 && l1iteminstance1.getUpdateCon() <= 10) {
							l1iteminstance1.setUpdateCon(l1iteminstance1.getUpdateCon() + 1); // 力 +1
							if (l1iteminstance1.isEquipped()) {
								pc.addCon(1);
								pc.sendPackets(new S_OwnCharStatus(pc));
							}
...
*/
				...
				} else if (itemId == 61000)// 于DB内自行设定此药水编号及相关设定
				{
					...
					if (pc.get_sex() == 0) {
						...
						L1World.getInstance().broadcastPacketToAll(
								new S_ServerMessage(166, "\\fY" + "【"
										+ pc.getName() + "】变成性感美女啰"));
					} else {
						pc.set_sex(0);
						L1World.getInstance().broadcastPacketToAll(
								new S_ServerMessage(166, "\\fX" + "【"
										+ pc.getName() + "】变成英俊帅哥啰"));
					}
					...
					pc.setClassId(newClassId);
					pc.sendPackets(new S_SkillSound(pcObjid, 5010));
					pc.broadcastPacket(new S_SkillSound(pcObjid, 5010));
					L1PolyMorph.doPoly(pc, newClassId, 1,
					...
					if(pc.getFamePoint()+ 5 > Config.FAME_LV11_POINT){
						pc.sendPackets(new S_ServerMessage(166, L1WilliamSystemMessage.ShowMessage(116)));
						return;
					}
					...
					if(pc.getFamePoint()+ 10 > Config.FAME_LV11_POINT){
						pc.sendPackets(new S_ServerMessage(166, L1WilliamSystemMessage.ShowMessage(116)));
						return;
					}
				...
				} else if (itemId == 49208) { // 蓝色之火碎片
					if (pc.isIllusionist() && (pc.getMapId() == 2004)) { // 异界
																			// 奎斯特
						boolean found = false;
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
				...
				} else if (itemId == 49227) { // 红色之火碎片
					if (pc.isDragonKnight() && (pc.getMapId() == 2004)) { // 异界
																			// 奎斯特
						boolean found = false;
						for (L1Object obj : L1World.getInstance().getObject()) {
							if (obj instanceof L1MonsterInstance) {
					...
					if (!L1CastleLocation.checkInAllWarArea(pc.getLocation())) {// 检查是否在城堡区域内
						pc.sendPackets(new S_DragonGate(pc, L1DragonSlayer
								.getInstance().checkDragonPortal()));
					} else {
...
//					sosodemon add 防具声望控制
	...
	private void UseArmor(L1PcInstance activeChar, L1ItemInstance armor) {
		...
		if (equipeSpace && !armor.isEquipped()) { // 使用した防具を装备していなくて、その装备个所が空いている场合（装着を试みる）
			...
			if (((type == 13) && (pcInventory.getTypeEquipped(2, 7) >= 1))
					|| ((type == 7) && (pcInventory.getTypeEquipped(2, 13) >= 1))) { // シールド、ガーダー同时装备不可
			...
			if ((type == 7) && (activeChar.getWeapon() != null)) { // シールドの场合、武器を装备していたら两手武器チェック
				if (activeChar.getWeapon().getItem().isTwohandedWeapon()) { // 两手武器
					activeChar.sendPackets(new S_ServerMessage(129)); // \f1两手の武器を武装したままシールドを着用することはできません。
					return;
				}
			}
			...
			if ((type == 3) && (pcInventory.getTypeEquipped(2, 2) >= 1)) { // シャツの场合、メイルを着てないか确认
				...
				return;
					...
					&& (pcInventory.getTypeEquipped(2, 4) >= 1)) { // シャツとメイルの场合、マントを着てないか确认
				activeChar.sendPackets(new S_ServerMessage(127)); // \f1それは脱ぐことができません。
				return;
			}
	...
	private void UseWeapon(L1PcInstance activeChar, L1ItemInstance weapon) {
		...
		if ((activeChar.getWeapon() == null)
				|| !activeChar.getWeapon().equals(weapon)) { // 指定された武器が装备している武器と违う场合、装备できるか确认
			...
			if (weapon.getItem().isTwohandedWeapon()
					&& (pcInventory.getTypeEquipped(2, 7) >= 1)) { // 两手武器の场合、シールド装备の确认
...
/*		if (((pcX > 33116) && (pcX < 33128) && (pcY > 32930) && (pcY < 32942) && (mapId == 4))
	...
	private void SpellBook1(L1PcInstance pc, L1ItemInstance l1iteminstance,
		...
		for (int j6 = 97; j6 < 112; j6++) {
			...
			if (Config.CLIENT_LANGUAGE == 3) {
				...
				s1 = (new StringBuilder()).append("闇精灵の水晶(")
						.append(l1skills.getName()).append(")").toString();
			}
			...
			if (l1iteminstance.getItem().getName().equalsIgnoreCase(s1)) {
				if (!pc.isGm() && (l1skills.getAttr() != 0)
					...
					if ((pc.getElfAttr() == 0) || (pc.getElfAttr() == 1)
							...
							|| (pc.getElfAttr() == 8)) { // 属性值が异常な场合は全属性を觉えられるようにしておく
						pc.sendPackets(new S_ServerMessage(79));
						return;
					}
				}
	...
	private boolean sendLetter(L1PcInstance pc, String name,
		...
		if (target != null) {
			if (target.getInventory().checkAddItem(item, 1) == L1Inventory.OK) {
				...
				target.sendPackets(new S_ServerMessage(428)); // 手纸が届きました。
			} else {
				if (isFailureMessage) {
	...
	private void makeCooking(L1PcInstance pc, int cookNo) {
		...
		if (cookNo == 0) { // フローティングアイステーキ
			if (pc.getInventory().checkItem(40057, 1)) {
				...
				if ((chance >= 1) && (chance <= 90)) {
					...
					pc.sendPackets(new S_ServerMessage(1101)); // 料理が失败しました。
		...
		} else if (cookNo == 16) { // クラスタシアンのハサミ烧き
