...
import l1j.plugin.L1GameRestart; // 自动重开系统
import java.util.ArrayList;// 连线线程
import l1j.plugin.L1IpCount;// 连线线程
import l1j.william.TeleportScroll; // DB道具传送卷轴(符) by 丫杰
...
public class GameServer extends Thread {
	// private ServerSocket _serverSocket; 连线线程
	private static Logger _log = Logger.getLogger(GameServer.class.getName());
	private static int YesNoCount = 0;
	// private int _port; 连线线程
	...
	// 连线线程 end
	// 连线线程 public void initialize() throws Exception {
	public void Load() throws Exception {
		...
		/*
		 * 连线线程 _port = Config.GAME_SERVER_PORT; if (!"*".equals(s)) {
		 ...
		 * System.out.println(L1Message.setporton + _port); }连线线程 end
		 */
		...
		if (!NpcTable.getInstance().isInitialized()) {
			throw new Exception("无法初始化NPC资料表");
		}
		...
		TeleportScroll.getInstance(); // DB道具传送卷轴(符) by 丫杰
	...
	private class ServerShutdownThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				world.broadcastServerMessage("伺服器即将爆炸。");
				System.out.println("伺服器即将关闭。");
				world.broadcastServerMessage("请玩家移动到安全区域准备逃生。");
				while (0 < secondsCount) {
					if (secondsCount <= 30) {
						world.broadcastServerMessage("伺服器将在" + secondsCount
								+ "秒后爆炸，请玩家移动到安全区域准备逃生。");
						System.out.println("伺服器将在" + secondsCount
								+ "秒后关闭。");
					} else {
				...
				world.broadcastServerMessage("爆炸危机已解除，伺服器将会继续正常运作！");
				System.out.println("取消关机动作，伺服器将会继续正常运作！");
				return;
			}
		}
	}
