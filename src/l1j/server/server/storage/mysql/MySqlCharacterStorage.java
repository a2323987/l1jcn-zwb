...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public L1PcInstance loadCharacter(String charName) {
		...
		try {
			...
			pc.setFamePoint(rs.getShort("FamePoint")); // sosodemon add 声望系统 BY.SosoDEmoN
			...
			pc.setPay(rs.getInt("Pay")); // 村庄福利金 此栏位由 HomeTownTimeController 处理 update
	...
	public void createCharacter(L1PcInstance pc) {
		...
		try {
			...
			pstm.setShort(++i, pc.getFamePoint()); // sosodemon add 声望系统 BY.SosoDEmoN
	...
	public void deleteCharacter(String accountName, String charName)
		...
		try {
			...
			if (!rs.next()) {
				/*
				 * SELECTが值を返していない
				 * 存在しないか、あるいは别のアカウントが所有しているキャラクター名が指定されたということになる。
				 */
