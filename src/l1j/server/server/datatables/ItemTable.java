...
public class ItemTable {
	...
	static {
		...
		_etcItemTypes.put("TeleportScroll", new Integer(18)); // 道具传送卷轴(符)
		_etcItemTypes.put("ItemBlend", new Integer(19)); // 道具融合系统
		_etcItemTypes.put("furniture", new Integer(20)); // 家具
		...
		_useTypes.put("spell_long", new Integer(5)); // 地面 / オブジェクト选択(远距离)
		...
		_useTypes.put("spell_short", new Integer(17)); // 地面 / オブジェクト选択(近距离)
		...
		_useTypes.put("spell_buff", new Integer(30)); // オブジェクト选択(远距离)
														// Ctrlを押さないとパケットが飞ばない？
		...
		// _useTypes.put("spell_long2", new Integer(39)); // 地面 / オブジェクト选択(远距离)
		// 5と同じ？
		_useTypes.put("earring", new Integer(40));
		_useTypes.put("fishing_rod", new Integer(42));
		// add 符石/遗物 : UseType= 43(右) 44(左-目前开放) 45(上) by srwh
		_useTypes.put("rune_r", new Integer(43));
		_useTypes.put("rune_l", new Integer(44));
		_useTypes.put("rune_m", new Integer(45));
		// end
		...
		// add 符石/遗物 : UseType= 43(右) 44(左-目前开放) 45(上) by srwh
		_armorTypes.put("rune_r", new Integer(14));
		_armorTypes.put("rune_l", new Integer(15));
		_armorTypes.put("rune_m", new Integer(16));
		// end
	...
	private Map<Integer, L1EtcItem> allEtcItem() {
		...
		try {
			...
			while (rs.next()) {
				...
				item.setItemDescExtra(rs.getString("itemdesc_extra")); // 新增可从DB自行输入鉴定描述
																		// by
																		// 13630020
			...
			_log.log(Level.SEVERE, new StringBuilder().append(item.getName()).append("(" + item.getItemId() + ")").append(" 载入失败。").toString());
		}
	...
	private Map<Integer, L1Weapon> allWeapon() {
		...
		try {
			...
			while (rs.next()) {
				...
				weapon.setItemDescExtra(rs.getString("itemdesc_extra")); // 新增可从DB自行输入鉴定描述
																			// by
																			// 13630020
			...
			_log.log(Level.SEVERE, new StringBuilder().append(weapon.getName()).append("(" + weapon.getItemId() + ")").append(" 载入失败。")
					.toString());
		}
		catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
	...
	private Map<Integer, L1Armor> allArmor() {
		...
		try {
			...
			while (rs.next()) {
				...
				armor.setItemDescExtra(rs.getString("itemdesc_extra")); // 新增可从DB自行输入鉴定描述
																		// by
																		// 13630020
			...
			_log.log(Level.SEVERE, new StringBuilder().append(armor.getName()).append("(" + armor.getItemId() + ")").append(" 载入失败。")
					.toString());
		}
		catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
