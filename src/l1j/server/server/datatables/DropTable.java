...
public class DropTable {
	...
	private final Map<Integer, List<L1Drop>> _droplists; // モンスター每のドロップリスト
...
/*		if (dropList == null) {
		...
		for (L1Drop drop : dropList) {
			...
			if ((adenarate == 0) && (itemId == L1ItemId.ADENA)) {
				continue; // アデナレート０でドロップがアデナの场合はスルー
			}
			...
			if (itemId == L1ItemId.ADENA) { // ドロップがアデナの场合はアデナレートを挂ける
				itemCount *= adenarate;
			}
			if (itemCount < 0) {
				itemCount = 0;
			...
			if (item.getItem().getType2() == 2) {
				...
				else if (item.getItem().get_safeenchant() == 0) {
					if (lvrnd <= 900) {					
	...
	public void dropShare(L1NpcInstance npc, List<L1Character> acquisitorList, List<Integer> hateList) {
		...
		for (int i = hateList.size() - 1; i >= 0; i--) {
			...
			if ((Config.AUTO_LOOT == 2) // オートルーティング２の场合はサモン及びペットは省く
		...
		for (int i = inventory.getSize(); i > 0; i--) {
			...
			boolean isGarbage = false; // 掉落物品过滤系统 by missu0524
			if ((item.getItem().getType2() == 0) && (item.getItem().getType() == 2)) { // light系アイテム
				item.setNowLighting(false);
			}

			if (((Config.AUTO_LOOT != 0) || (itemId == L1ItemId.ADENA)) && (totalHate > 0)) { // オートルーティングかアデナで取得者がいる场合
						...
						if (acquisitor.getInventory().checkAddItem(item, item.getCount()) == L1Inventory.OK) {
							...
							if (acquisitor instanceof L1PcInstance) {
								...
								if ((l1iteminstance != null) && (l1iteminstance.getCount() > 2000000000)) {
									...
									isGround = true;
									player.sendPackets(new S_ServerMessage(166, "所持有的金币", "超过了2,000,000,000上限")); // \f1%0が%4%1%3%2
								}
								else {
									if (player.isInParty()) { // パーティの场合
										partyMember = player.getParty().getMembers();
										//组队自动分配 (item.getCount() > 现场队员人数才分配，分配后剩余数量 otherCount 归第一顺位队员所有(应该是杀死怪的人))
										if (player.getPartyType() == 1) {
											...
											if (partySize > 1 && item.getCount() >= partySize) {
												...
												fi: // 掉落物品过滤系统 by missu0524
													for (L1PcInstance member : partyMember) {
														if (member !=null && member.getMapId() == npc.getMapId() 
																&& member.getCurrentHp() > 0 && !member.isDead()) {
															// 掉落物品过滤系统 by missu0524
															if (member.checkFilterList(itemId)) {
																continue fi;
															}
															// end
										...
										// ソロの场合
										// 掉落物品过滤系统 by missu0524
										if (player.checkFilterList(itemId)) {
											targetInventory = null;
											isGarbage = true;
											break;
										}
			...
			// 掉落物品过滤系统 by missu0524
			if(isGarbage == true && targetInventory == null){
				inventory.removeItem(item, item.getCount());
				continue;
			}
			// end
