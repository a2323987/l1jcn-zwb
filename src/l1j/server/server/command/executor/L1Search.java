			...
			while (st.hasMoreTokens()) {
				...
				if (itCount == 0
						&& (tempVar.equals("防具") || tempVar.equals("武器")
								|| tempVar.equals("道具") || tempVar.equals("变身") || tempVar
								.equals("NPC"))) {
					simpleS = false;
					type = tempVar;
				} else {
	...
	private void find_object(L1PcInstance pc, String type, String name) {
		try {
			...
			if (error == false) {
				...
				pc.sendPackets(new S_SystemMessage("正在搜寻符合 '"
						+ name.replace("%", " ") + " ' 的" + type + "名称..."));
				while (rs.next()) {
					str1 = rs.getString(1);
					str2 = rs.getString(2);
					if (type.equals("防具") || type.equals("武器")
						...
						pc.sendPackets(new S_SystemMessage(blessed + "编号: "
								+ str1 + ", " + str2));
					} else {
						pc.sendPackets(new S_SystemMessage("编号: " + str1 + ", "
								+ str2));
					}
					count++;
				}
				...
				statement.close();
				con.close();
				pc.sendPackets(new S_SystemMessage("找到 " + count + " 个物件符合"
						+ type + "类型。"));
			}
		} catch (Exception e) {
		}
	...
	private void find_object(L1PcInstance pc, String name) {
		try {
			...
			while (rs.next()) {
				...
				pc.sendPackets(new S_SystemMessage(blessed + "编号: " + str1
						+ ", " + str2));
				count1++;
			}
			rs.close();
