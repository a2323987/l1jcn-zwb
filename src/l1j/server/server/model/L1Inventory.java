...
public class L1Inventory extends L1Object {
	...
	public int checkAddItem(L1ItemInstance item, int count) {
		...
		if (weight > (MAX_WEIGHT * Config.RATE_WEIGHT_LIMIT_PET)) { // その他の重量确认（主にサモンとペット）
			return WEIGHT_OVER;
		}
	...
	public synchronized L1ItemInstance storeItem(int id, int count) {
		...
		if (id == 40312) {
			...
			if (findKeyId(id) == null) { // 新しく生成する必要がある场合のみIDの発行とL1Worldへの登录を行う
			...
			if (findItemId(id) == null) { // 新しく生成する必要がある场合のみIDの発行とL1Worldへの登录を行う
				item.setId(IdFactory.getInstance().nextId());
				L1World.getInstance().storeObject(item);
			}
	 ...
	 * がある场合はremoveItemの方を使用するのがよい。 （こちらは矢とか魔石とか特定のアイテムを消费させるときに使う）
	 * 
	 * @param itemid
	 *            - 削除するアイテムのitemid(objidではない)
	 * @param count
	 *            - 削除する个数
	 * @return 实际に削除された场合はtrueを返す。
	 */
	public boolean consumeItem(int itemid, int count) {
		...
		if (ItemTable.getInstance().getTemplate(itemid).isStackable()) {
			...
			if (itemList.length == count) {
				...
				return true;
			} else if (itemList.length > count) { // 指定个数より多く所持している场合
				DataComparator<L1ItemInstance> dc = new DataComparator<L1ItemInstance>();
				Arrays.sort(itemList, dc); // エンチャント顺にソートし、エンチャント数の少ないものから消费させる
				for (int i = 0; i < count; i++) {
					removeItem(itemList[i], 1);
				}
				return true;
	...
	// 指定したアイテムから指定个数を削除（使ったりゴミ箱に舍てられたとき）戻り值：实际に削除した数
	...
	public int removeItem(L1ItemInstance item, int count) {
		...
		if (item.getCount() == count) {
			...
			} else if ((itemId >= 41383) && (itemId <= 41400) || (itemId >= 49065) && (itemId <= 49076)) { // 家具(新增)
	...
	// 特定のアイテムを指定された个数以上所持しているか确认（矢とか魔石の确认）
	...
	// 强化された特定のアイテムを指定された个数以上所持しているか确认
	// 装备中のアイテムは所持していないと判别する
	...
	/**
	 * スタック可能なアイテムリストからnameIdと同じ值を持つitemを返す
	 * 
	 * @param nameId
	 * @return item null 如果没有找到。
	 */
