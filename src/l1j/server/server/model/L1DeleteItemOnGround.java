...
public class L1DeleteItemOnGround {
	...
	public void initialize() {
		...
		GeneralThreadPool.getInstance().execute(_deleteTimer); // タイマー开始
	}
	...
	private void deleteItem() {
		...
		for (L1Object obj : L1World.getInstance().getObject()) {
			...
			if (item.getItem().getItemId() == 40515) { // 精灵の石
				continue;
			}
			...
			if (players.isEmpty()) { // 指定范围内にプレイヤーが居なければ削除
		...
		_log.fine("自动删除了地上杂物。删除数量: " + numOfDeleted);
	}
}
