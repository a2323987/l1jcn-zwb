...
public class L1War {
	...
	class CastleWarTimer implements Runnable {
		...
		public void run() {
			for (;;) {
				...
				if (_isWarTimerDelete) { // 战争が终结していたらタイマー终了
					return;
				}
			}
	...
	public void handleCommands(int war_type, String attack_clan_name, String defence_clan_name) {
		...
		if (war_type == 1) { // 攻城战
			...
			GeneralThreadPool.getInstance().execute(castle_war_timer); // タイマー开始
		}
				...
				for (L1PcInstance element : clan2_member) {
					if (type == 1) { // 宣战布告
						...
						if (attack_clan_num == 1) { // 攻击侧クランが一つ
							...
							element.sendPackets(new S_ServerMessage( // %0血盟と%1血盟间の战争が终结しました。
	...
	public void WinCastleWar(String clan_name) { // クラウンを夺取して、攻击侧クランが胜利
		String defence_clan_name = GetDefenceClanName();
		L1World.getInstance().broadcastPacketToAll(new S_ServerMessage( // %0血盟が%1血盟との战争で胜利しました。
		...
		for (String element : clanList) {
			if (element != null) {
				L1World.getInstance().broadcastPacketToAll(new S_ServerMessage( // %0血盟と%1血盟间の战争が终结しました。
	...
	public void CeaseCastleWar() { // 战争时间满了し、防卫侧クランが胜利
		String defence_clan_name = GetDefenceClanName();
		String clanList[] = GetAttackClanList();
	...
	public boolean CheckClanInWar(String clan_name) { // クランが战争に参加しているかチェックする
		boolean ret;
	...
	public void delete() {
		L1World.getInstance().removeWar(this); // 战争リストから削除
	}
