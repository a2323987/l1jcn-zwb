...
*/
		...
		if ((_calcType == PC_PC) && (cha instanceof L1PcInstance)) {
			if (((_skill.getTargetTo() & L1Skills.TARGET_TO_CLAN) == L1Skills.TARGET_TO_CLAN) && (((_player.getClanid() != 0 // ターゲットがクラン员
	...
	private boolean isAttrAgrees() {
		...
		if ((_skill.getSkillLevel() >= 17) && (_skill.getSkillLevel() <= 22) && (magicattr != 0 // 精灵魔法で、无属性魔法ではなく、
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (_skill.getTargetTo() == L1Skills.TARGET_TO_ME) {//怪物自补时，效果目标 = 自己
				...
				switch(_skillId) {
					...
					case HASTE:
						if (cha.getMoveSpeed() != 2) { // スロー中以外
							if (cha instanceof L1PcInstance) {
								L1PcInstance pc = (L1PcInstance) cha;
								if (pc.getHasteItemEquipped() > 0 || pc.getHasteItemByDoll() > 0) { // TODO 加速状态魔法娃娃
									continue;
								}
								pc.setDrink(false);
								pc.sendPackets(new S_SkillHaste(pc.getId(), 1, _getBuffIconDuration));
