...
class L1SkillStop {
	public static void stopSkill(L1Character cha, int skillId) {
		...
		else if (skillId == ELEMENTAL_FALL_DOWN) { // 弱化属性
		...
		else if (skillId == STATUS_FREEZE) { // 束缚
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		else if (skillId == STATUS_CUBE_IGNITION_TO_ENEMY) { // キューブ[イグニション]：敌
		}
		else if (skillId == STATUS_CUBE_QUAKE_TO_ENEMY) { // キューブ[クエイク]：敌
		}
		else if (skillId == STATUS_CUBE_SHOCK_TO_ENEMY) { // キューブ[ショック]：敌
		}
		...
		else if (skillId == STATUS_UNDERWATER_BREATH) { // エヴァの祝福＆マーメイドの鳞
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		// ****** 自订状态区
		else if (skillId == LOGIN_EIN_TIME) { // 殷海萨的祝福
			if (Config.EINHASAD_IS_ACTIVE) {
				if (cha instanceof L1PcInstance) {
					L1PcInstance pc = (L1PcInstance) cha;
					if (pc.isMatchEinResult()) {
						pc.addEinPoint(1); // 点数 +1
					}
				}
				cha.setSkillEffect(L1SkillId.LOGIN_EIN_TIME,
						Config.EIN_TIME * 60000); // N分钟
			}
		}
		else if (skillId == LOGIN_TIME_GET_ITEM) { // 在线一段时间后给予道具 by 丫杰
		...
		else if ((skillId == COOKING_1_2_N) || (skillId == COOKING_1_2_S)) { // ナッツ饼
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		else if ((skillId == COOKING_1_3_N) || (skillId == COOKING_1_3_S)) { // 蚁脚のチーズ烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addAc(1);
		...
		else if ((skillId == COOKING_1_6_N) || (skillId == COOKING_1_6_S)) { // 猪肉の串烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addMr(-5);
				pc.sendPackets(new S_SPMR(pc));
		...
		else if ((skillId == COOKING_2_3_N) || (skillId == COOKING_2_3_S)) { // キウィパロット烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		else if ((skillId == COOKING_2_4_N) || (skillId == COOKING_2_4_S)) { // スコーピオン烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		else if ((skillId == COOKING_2_6_N) || (skillId == COOKING_2_6_S)) { // クモ脚の串烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addSp(-1);
				pc.sendPackets(new S_SPMR(pc));
		...
		else if ((skillId == COOKING_3_0_N) || (skillId == COOKING_3_0_S)) { // クラスタシアンのハサミ烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
		...
		else if ((skillId == COOKING_3_1_N) || (skillId == COOKING_3_1_S)) { // グリフォン烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addMaxHp(-50);
				pc.sendPackets(new S_HPUpdate(pc.getCurrentHp(), pc.getMaxHp()));
		...
		else if ((skillId == COOKING_3_3_N) || (skillId == COOKING_3_3_S)) { // タートルドラゴン烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addAc(3);
		...
		else if ((skillId == COOKING_3_5_N) || (skillId == COOKING_3_5_S)) { // ドレイク烧き
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addSp(-2);
				pc.sendPackets(new S_SPMR(pc));
		...
		else if ((skillId == COOKING_3_6_N) || (skillId == COOKING_3_6_S)) { // 深海鱼のシチュー
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.addMaxHp(-30);
				pc.sendPackets(new S_HPUpdate(pc.getCurrentHp(), pc.getMaxHp()));
