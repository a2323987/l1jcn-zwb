...
public class L1Spawn extends L1GameTimeAdapter {
	...
	private int calcRespawnDelay() {
		...
		if ((_time != null) && !_time.getTimePeriod().includes(currentTime)) { // 指定时间外なら指定时间までの时间を足す
	...
	protected void doSpawn(int spawnNumber, int objectId) { // 再出现
		...
		try {
			...
			// 随机面向 by 9001183ex
			else if (getHeading() == 9) { // 在spawnlist中的heading栏位输入9就会随机面向了。
			...
			else if ((npcId == 45601) && (getMapId() == 11)) { // 死亡骑士
				mob.setMap((short) (getMapId() + Random.nextInt(3)));
			}
			else if ((npcId == 81322) && (getMapId() == 25)) { // 黑骑士副队长
				mob.setMap((short) (getMapId() + Random.nextInt(2)));
			}
			else {
				mob.setMap(getMapId());
			...
			while (tryCount <= 50) {
				switch (getSpawnType()) {
					case SPAWN_TYPE_PC_AROUND: // PC周辺に涌くタイプ
						if (!_initSpawn) { // 初期配置では无条件に通常spawn
						...
						// フロアにPCがいなければ通常の出现方法
					default:
						if (isAreaSpawn()) { // 座标が范围指定されている场合
							Point pt = null;
							if (_spawnHomePoint && (null != (pt = _homePoint.get(spawnNumber)))) { // ホームポイントを元に再出现させる场合
							...
							if (tryCount > 49) { // 出现位置が决まらない时はlocx,locyの值
								newlocx = getLocX();
								newlocy = getLocY();
							}
						}
						else if (isRandomSpawn()) { // 座标のランダム值が指定されている场合
				...
				if (mob.getMap().isInMap(mob.getLocation()) && mob.getMap().isPassable(mob.getLocation())) {
					if (mob instanceof L1MonsterInstance) {
						...
						// 画面内にPCが居て出现できない场合は、3秒后にスケジューリングしてやり直し
			...
			if (_initSpawn && _spawnHomePoint) { // 初期配置でホームポイントを设定
				Point pt = new Point(mob.getX(), mob.getY());
				_homePoint.put(spawnNumber, pt); // ここで保存したpointを再出现时に使う
			}
			...
			if ((npcId == 81341) && ((mob.getMapId() == 2000) || (mob.getMapId() == 2001) || (mob.getMapId() == 2002) || (mob.getMapId() == 2003))) { // 再生之祭坛
				for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
