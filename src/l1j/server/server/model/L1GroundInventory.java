		...
		public void run() {
			try {
				synchronized (L1GroundInventory.this) {
					if (!_items.contains(_item)) {// 拾われたタイミングによってはこの条件を满たし得る
						return; // 既に拾われている
					}
					removeItem(_item);
				}
	...
	public void onPerceive(L1PcInstance perceivedFrom) {
		for (L1ItemInstance item : getItems()) {
			if (!perceivedFrom.knownsObject(item)) {
				...
				perceivedFrom.sendPackets(new S_DropItem(item)); // プレイヤーへDROPITEM情报を通知
			}
		}
	}

	// 认识范围内にいるプレイヤーへオブジェクト送信
	@Override
