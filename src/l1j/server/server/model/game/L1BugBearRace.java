...
public class L1BugBearRace {
	...
	public boolean checkPosition(int runnerNumber) {// 现在のポジションを确认
		...
		int x = npc.getX();
		int y = npc.getY();
		if (_runnerStatus[runnerNumber] == 0) {// スタート　直线
			...
			(x >= 33476 && x <= 33476 + 8) && (y >= 32861 && y <= 32861 + 8)) {
				...
				npc.setHeading(defaultHead[_runnerStatus[runnerNumber]]);// ヘッジを变更
			} else {
				npc.setHeading(defaultHead[_runnerStatus[runnerNumber]]);// ヘッジを复元
			}
	...
	private class RaceTimer extends TimerTask {
		...
		public void run() {
			...
			try {
				...
				ticket.set_itemobjid(FIRST_ID);// 重复可能
				...
				RaceTicketTable.getInstance().oldTicketDelete(getRound());// 古い记录を削除
				/**/
				setRandomRunner();// ランナー准备
				setRandomCondition();
				/* SHOP BuyList格纳处理 */
				...
				for (int i = 0; i < 5; i++) {
					//L1ShopItem shopItem1 = new L1ShopItem(40309, 500, 1, 0, null); // 道具天数删除系统
					L1ShopItem shopItem1 = new L1ShopItem(40309, 500, 1, 0, null,0,0,0); // 道具+几系统
					shopItem1.setName(i);
					//L1ShopItem shopItem2 = new L1ShopItem(40309, 500, 1, 0, null); // 道具天数删除系统
					L1ShopItem shopItem2 = new L1ShopItem(40309, 500, 1, 0, null,0,0,0); // 道具+几系统
					shopItem2.setName(i);
					//L1ShopItem shopItem3 = new L1ShopItem(40309, 500, 1, 0, null); // 道具天数删除系统
					L1ShopItem shopItem3 = new L1ShopItem(40309, 500, 1, 0, null,0,0,0); // 道具+几系统
					shopItem3.setName(i);
	 ...
	 * @param heading
	 *            向き
	 * @return L1NpcInstance 戻り值 : 成功=生成したインスタンス 失败=null
	 */
