...
public class L1Location extends Point {
	...
	/**
	 * このLocationに对する、移动可能なランダム范围のLocationを返す。
	 * ランダムテレポートの场合は、城エリア、アジト内のLocationは返却されない。
	 * 
	 * @param max
	 *            ランダム范围の最大值
	...
	/**
	 * 引数のLocationに对して、移动可能なランダム范围のLocationを返す。
	 * ランダムテレポートの场合は、城エリア、アジト内のLocationは返却されない。
	 * 
	 * @param baseLocation
	 *            ランダム范围の元になるLocation
	...
	public static L1Location randomLocation(L1Location baseLocation, int min, int max, boolean isRandomTeleport) {
		if (min > max) {
			throw new IllegalArgumentException("min > max的参数无效");
		}
		if (max <= 0) {
		...
		while (true) {
			...
			if (isRandomTeleport) { // ランダムテレポートの场合
