...
public class L1CrownInstance extends L1NpcInstance {
	...
	public void onAction(L1PcInstance player) {
		...
		List<L1War> wars = L1World.getInstance().getWarList(); // 全战争リストを取得
		for (L1War war : wars) {
			if (castle_id == war.GetCastleId()) { // 今居る城の战争
				in_war = war.CheckClanInWar(playerClanName);
				break;
			}
		}
		if (existDefenseClan && (in_war == false)) { // 城主が居て、布告していない场合
			return;
		}
