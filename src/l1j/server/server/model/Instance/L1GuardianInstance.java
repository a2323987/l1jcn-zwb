...
public class L1GuardianInstance extends L1NpcInstance {
	...
	public void searchTarget() {
		...
		for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(this)) {
			...
			if (!pc.isInvisble() || getNpcTemplate().is_agrocoi()) { // インビジチェック
				if (!pc.isElf()) { // エルフ以外
					targetPlayer = pc;
					wideBroadcastPacket(new S_NpcChatPacket(this, "$804", 2)); // エルフ以外の者よ、命が惜しければ早くここから去れ。ここは神圣な场所だ。
					break;
				} else if (pc.isElf() && pc.isWantedForElf()) {
					targetPlayer = pc;
					wideBroadcastPacket(new S_NpcChatPacket(this, "$815", 1)); // 同族を杀したものは、己の血でその罪をあがなうことになるだろう。
					break;
				}
			}
		}
				...
				try {
					...
					if (npcId == 70848) { // 安特
						if (_inventory.checkItem(40499)
								&& !_inventory.checkItem(40505)) { // 蘑菇汁 换
																	// 安特之树皮
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			int pcx = player.getX(); // PCのX座标
			int pcy = player.getY(); // PCのY座标
			int npcx = target.getX(); // NPCのX座标
			int npcy = target.getY(); // NPCのY座标
