...
//		add william sosodemon end
		...
		if (talking != null) {
			if (npcid == 70841) { // ルーディエル
			...
			} else if (npcid == 81335) { // 被遗弃的肉身
				if (player.getInventory().checkItem(49241, 1)) {
					htmlid ="50q_pout1";
				} else {
					htmlid = "50q_pout";
				}
			} else if (npcid == 81345) { // 往圣殿的入口
				int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
				int lv50_step = quest.get_step(L1Quest.QUEST_LEVEL50);
				if ((player.getLevel() >= 50) && (lv45_step == L1Quest.QUEST_END)) {
				...
				if (player.isDarkelf()) { // 黑暗妖精
					if (player.getLevel() >= 45) {
						if (quest.isEnd(L1Quest.QUEST_LEVEL30)) { // 30级任务结束
							int lv45_step = quest
									.get_step(L1Quest.QUEST_LEVEL45);
				...
				if (player.isDragonKnight()) { // 龙骑士 LV50 试练任务
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					int lv50_step = quest.get_step(L1Quest.QUEST_LEVEL50);
				...
				if (player.isIllusionist()) { // 幻术士 LV50 试练任务
					int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
					int lv50_step = quest.get_step(L1Quest.QUEST_LEVEL50);
					if (player.getLevel() > 49
							&& lv45_step == L1Quest.QUEST_END) {
