...
import java.util.HashSet; // 掉落物品过滤系统 by missu0524
...
import l1j.server.server.serverpackets.S_SkillIconEinhasad; // 殷海萨的祝福
import l1j.server.server.serverpackets.S_SkillHaste; // TODO 加速状态魔法娃娃
...
public class L1PcInstance extends L1Character {
	...
	// TODO 加速状态魔法娃娃
	public void startHasteByDoll() {
		if (getHasteItemEquipped() == 1) { // 判断是否有装备加速
			setHasteItemEquipped(0); // 移除
		}
		setHasteItemByDoll(1);
		removeHasteSkillEffect();
		if (getMoveSpeed() != 1 && getHasteItemByDoll() == 1) {
			setMoveSpeed(1);
			sendPackets(new S_SkillHaste(getId(), 1, -1));
			broadcastPacket(new S_SkillHaste(getId(), 1, 0));
		}
	}
	
	public void stopHasteByDoll() {
		setHasteItemByDoll(0);
		if (getHasteItemByDoll() == 0) { // 判断是否有装备加速
			setMoveSpeed(0);
	...
	public void setCurrentMp(int i) {
		...
		int currentMp = i;
		if ((currentMp >= getMaxMp()) || ((isGm()) && (Config.GM_Decrease_MP))) {// GM使用魔法是否扣魔4/4
			currentMp = getMaxMp();
		}
	...
	public void logout() {
		...
		//add GUI by Eric
		if(Config.GUI)
			l1j.gui.Eric_J_Main.getInstance().delPlayerTable(getName());//GUI
		//end
	}
	...
	public void receiveDamage(L1Character attacker, double damage, boolean isMagicDamage) { // 攻击でＨＰを减らすときはここを使用
		if ((getCurrentHp() > 0) && !isDead()) {
			...
			System.out.println("警告∶角色的ＨＰ减少处理有不正确的地方。※或者从最初开始ＨＰ０﹝L1PcInstance﹞");
			death(attacker);
		}
	}
	...
	private void levelUp(int gap) {
		...
		if (Config.AUTO_ADD_SKILL) {
			l1j.plugin.AutoLearnSkill.forAutoLearnSkill(this);
		}
		// 升级奖励道具
		if (Config.LEVEL_UP_REWARD) {
	...
	public void setHasteItemEquipped(int i) {
		_hasteItemEquipped = i; // 修正累加错误
	}
	
	// TODO 加速状态魔法娃娃
	private int _hasteItemByDoll = 0;

	public int getHasteItemByDoll() {
		return _hasteItemByDoll;
	}

	public void setHasteItemByDoll(int i) {
		_hasteItemByDoll = i;
	}
	// emd
	...
	public boolean isEinZone() { // 判断是否在安全区域
		return getMap().isSafetyZone(getLocation());
	}

	public boolean isEinLevel() { // 判断是否符合等级
		return _isEinLevel;
	}
	...
	private int _einPoint = 0; // 殷海萨的祝福 - %数
	private int _ein_getExp = 0; // 殷海萨的祝福 - 取得经验点数的累计
	private static final int einMaxPercent = Config.EIN_MAX_PERCENT; // 殷海萨的祝福

	// 百分比的最大值

	// 将打怪所获得的经验值做暂时的累积 并提供 置换功能
	...
	public void checkEinhasad() {
		if (Config.EINHASAD_IS_ACTIVE) {
			setEinLevel();
			if (isMatchEinResult()) { // 49级以上 殷海萨的祝福纪录
				addEinPoint(0);
			}
		}
	}
	...
	// 掉落物品过滤系统 by missu0524
	public HashSet<Integer> filterList = new HashSet<Integer>();

	public String getFilterList() {
		String s = "";
		for (Integer i : filterList) {
			s += ItemTable.getInstance().getTemplate(i).getName()+",";
		}
		return s;
	}

	public boolean checkFilterList(int id) {
		return filterList.contains(id);
	}

	public void addFilterList(int id) {
		filterList.add(id);
	}

	public void deleteFilterList(int id) {
