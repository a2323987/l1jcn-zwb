...
import l1j.server.server.serverpackets.S_HPUpdate; // TODO 魔法娃娃效果 - 增加HP 增加MP
...
public class L1DollInstance extends L1NpcInstance {
	...
	public L1DollInstance(L1Npc template, L1PcInstance master, int itemId,
		...
		// TODO 加速状态魔法娃娃
		if (L1MagicDoll.isHasteByDoll(_master)) {
			master.startHasteByDoll();
		}
		// end
		// TODO 魔法娃娃效果 - 增加HP 增加MP
		master.addMaxHp(L1MagicDoll.getAddHpByDoll(master));
		master.addMaxMp(L1MagicDoll.getAddMpByDoll(master));
		master.sendPackets(new S_HPUpdate(master.getCurrentHp(), master.getMaxHp()));
		if (master.isInParty()) { // 组队中
			master.getParty().updateMiniHP(master);
		}
		// end
	...
	public void deleteDoll() {
		...
		// TODO 加速状态魔法娃娃
		if (L1MagicDoll.isHasteByDoll(_master)) {
			((L1PcInstance) _master).stopHasteByDoll();
		}
		// end
		// TODO 魔法娃娃效果 - 增加HP 增加MP
		((L1PcInstance) _master).addMaxHp(-(L1MagicDoll.getAddHpByDoll(((L1PcInstance) _master))));
		((L1PcInstance) _master).addMaxMp(-(L1MagicDoll.getAddMpByDoll(((L1PcInstance) _master))));
		((L1PcInstance) _master).sendPackets(new S_HPUpdate(((L1PcInstance) _master).getCurrentHp(), ((L1PcInstance) _master).getMaxHp()));
		if (((L1PcInstance) _master).isInParty()) { // 组队中
			((L1PcInstance) _master).getParty().updateMiniHP(((L1PcInstance) _master));
		}
		// end
