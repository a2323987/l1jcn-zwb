...
public class L1TowerInstance extends L1NpcInstance {
	...
	public void receiveDamage(L1Character attacker, int damage) { // 攻击でＨＰを减らすときはここを使用
		if (_castle_id == 0) { // 初期设定で良いがいい场所がない
			if (isSubTower()) {
		...
		if ((_castle_id > 0) && WarTimeController.getInstance().isNowWar(_castle_id)) { // 战争时间内
			...
			// アデン城のメインタワーはサブタワーが3つ以上破坏されている场合のみ攻击可能
			...
			for (L1War war : L1World.getInstance().getWarList()) {
				if (_castle_id == war.GetCastleId()) { // 今居る城の战争
					isProclamation = war.CheckClanInWar(pc.getClanname());
					break;
				}
			}
			if ((existDefenseClan == true) && (isProclamation == false)) { // 城主が居て、布告していない场合
				return;
			}
