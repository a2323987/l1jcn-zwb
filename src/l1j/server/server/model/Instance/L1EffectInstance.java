...
public class L1EffectInstance extends L1NpcInstance {
	...
	private static final int CUBE_INTERVAL = 500; // キューブ范围内に居るキャラクターをチェックする间隔

	private static final int CUBE_TIME = 8000; // 效果时间8秒?
				...
				try {
					for (L1Object objects : L1World.getInstance().getVisibleObjects(_effect, 3)) {
						if (objects instanceof L1PcInstance) {
							...
							if (pc.getZoneType() == 1) { // セーフティーゾーンでは战争中を除き敌には无效
								boolean isNowWar = false;
								int castleId = L1CastleLocation.getCastleIdByArea(pc);
								if (castleId > 0) {
									isNowWar = WarTimeController.getInstance().isNowWar(castleId);
