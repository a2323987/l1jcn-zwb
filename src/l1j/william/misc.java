	...
	public static void forRequestItemUSe(ClientThread client,L1ItemInstance itemInstance){
		...
		for(int i=0;i<aData8.size();i++){
	          ...
	          if( itemid == ((Integer) ((ArrayList) aData8.get(i)).get(0)).intValue() ){
				  ...
				  user.sendPackets(new S_SystemMessage( (new java.lang.StringBuilder().append(resource.getString("weapon.upgrade.message2")  + misc.getWeaponExp( itemInstance.getId())+"/"+ ((Integer) ((ArrayList) aData8.get(i)).get(1)).intValue()  )).toString()));
				  break;
	          }
		}		
		//System.out.println("data5 size=" + aData5.size());
		for(int i=0;i<aData5.size();i++){
			aTempData = (int[]) aData5.get(i);
			iX1 = aTempData[4];
			iY1 = aTempData[5];
			iX2 = aTempData[6];
			iY2 = aTempData[7];
...
////// 以下是兑换活动
	public static ArrayList forRequestNPCAction(String s,L1PcInstance l1pcinstance) {
		ArrayList aTempData = null;
		ArrayList aTempData8 = null;
		...
		if( s.equalsIgnoreCase("upgrade_weapon") ){
			...
			for (Object itemObject :  l1pcinstance.getInventory().getItems()) {
				...
				if(item.getItem().getType2() == 1 && item.isEquipped()) {
					...
					for(int i=0;i<aData8.size();i++){
					  aTempData8=(ArrayList)aData8.get(i);
		...
		for(int i=0;i<aData.size();i++){
			aTempData = (ArrayList) aData.get(i);
  	        if( ((String)aTempData.get(7)).equals(s) ) {
				...
				int iMonth = aC.get(aC.MONTH)+1;
				int iDay = aC.get(aC.DAY_OF_MONTH );
				String sDate = null;
				int iTime = 0;
				int iType = Integer.parseInt((String)aTempData.get(11));
				int iComparedNum =0;
				if(iMonth<10) 
					  sDate = aC.get(aC.YEAR)+"0"+iMonth;
					else
				      sDate = aC.get(aC.YEAR)+""+iMonth;
				if(iDay<10) 
					  sDate = sDate + "0" + iDay;
					else
					  sDate = sDate + "" + iDay;
			    iTime = aC.get(aC.HOUR_OF_DAY);
    ...
    private static void upgradeWeapon(int weapon_oid,int new_weapon_id,String new_weapon_name){
	      java.sql.Connection con = null;	   
	        try { 
	          ...
	          try{
	        //sosodemon 移除不必要的更新
	        // 	    stat.executeUpdate("update character_items set item_id='"+ new_weapon_id + "',item_name='"+new_weapon_name+"' where id ='"+weapon_oid+"'");
   ...
   private static Object getArray(String s,String sToken,int iType){
		...
		try {
			...
			if (l1npc != null) {
				...
				try {
					...
					l1npcinstance.setId(IdFactory.getInstance()
							.nextId());
    ...
    private static boolean consumeItem(L1PcInstance l1pcinstance,int[] materials,int[] counts){
			...
			if (isCreate) {
				...
				for (int j = 0; j < materials.length; j++) {
					// 材料消费
					l1pcinstance.getInventory().consumeItem(materials[j],
							counts[j]);
				}
