...
public class L1WilliamItemSummon {
	...
	private static Logger _log = Logger
			.getLogger(L1WilliamItemSummon.class.getName());
	...
	public static void getItemSummon(L1PcInstance pc, L1ItemInstance l1iteminstance, int itemId) {
		...
		if (Item_Summon.getCheckClass() != 0) { // 职业判断
			...
			switch(Item_Summon.getCheckClass())
			{
				...
				break;
				case 6:
					msg = "龙骑士"; // 龙骑士 sosodemon add
				break;
				case 7:
					msg = "幻术师"; // 幻术师 sosodemon add
				break;
			...
			if (Item_Summon.getCheckClass() != class_id) { // 职业不符
				pc.sendPackets(new S_ServerMessage(166, L1WilliamSystemMessage.ShowMessage(1097)
					, msg, L1WilliamSystemMessage.ShowMessage(1114)));
				return;
			}
		}

		if (pc.getCha() < Item_Summon.getSummonCost()) { // 魅力不足
			pc.sendPackets(new S_ServerMessage(166, L1WilliamSystemMessage.ShowMessage(1098) + " {" + Item_Summon.getSummonCost() + ")"));
			return;
		}
		...
		if (Item_Summon.getCheckItem() != 0) { // 携带物品判断
			if (!pc.getInventory().checkItem(Item_Summon.getCheckItem())) {
				...
				pc.sendPackets(new S_ServerMessage(166, L1WilliamSystemMessage.ShowMessage(1113) 
					, " (" + temp.getName() + ") "
					, L1WilliamSystemMessage.ShowMessage(1114)));
				return;
			}
		}
		...
		if (Item_Summon.getHpConsume() != 0 || Item_Summon.getMpConsume() != 0) { // 体力 && 魔力判断
			if (pc.getCurrentHp() < Item_Summon.getHpConsume()) {
				pc.sendPackets(new S_ServerMessage(166, "$1083"
					, L1WilliamSystemMessage.ShowMessage(1113) + " (" + Item_Summon.getHpConsume() + ") "
					, L1WilliamSystemMessage.ShowMessage(1114)
					, L1WilliamSystemMessage.ShowMessage(1115)));
				return;
			}
			if (pc.getCurrentMp() < Item_Summon.getMpConsume()) {
				pc.sendPackets(new S_ServerMessage(166, "$1084"
					, L1WilliamSystemMessage.ShowMessage(1113) + " (" + Item_Summon.getMpConsume() + ") "
					, L1WilliamSystemMessage.ShowMessage(1114)
					, L1WilliamSystemMessage.ShowMessage(1115)));
				return;
			}
		...
		if (pc.getMap().isRecallPets()) {
			...
			if (summonCount == 0) {
				if (petcost > 0) { // 已有召唤怪
					...
					pc.sendPackets(new S_ServerMessage(166, L1WilliamSystemMessage.ShowMessage(1098) + " {" + summonCost + ")"));
				}
