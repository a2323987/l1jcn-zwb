...
public class MySqlCharacterStorage implements CharacterStorage {
    ...
    public void createCharacter(final L1PcInstance pc) {
        ...
        try {
            ...
            pstm = con
                    .prepareStatement("INSERT INTO characters SET account_name=?,objid=?,char_name=?,birthday=?,level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,PkCountForElf=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,Pay=?,HellTime=?,Banned=?,Karma=?,LastPk=?,LastPkForElf=?,DeleteTime=?,RocksPrisonTime=?");
    ...
    public L1PcInstance loadCharacter(final String charName) {
        ...
        try {
             ...
             * locY = 33392; map = 4; break; case 2: // ti locX = 32580; locY =
             * 32931; map = 0; break; case 3: // wb locX = 32621; locY = 33169;
             * map = 4; break; case 4: // kent locX = 33050; locY = 32780; map =
             * 4; break; case 5: // h locX = 33612; locY = 33268; map = 4;
             * break; default: // skt locX = 33080; locY = 33392; map = 4;
             * break; } } pc.set_x(locX); pc.set_y(locY); pc.set_map(map);
             */
            ...
            pc.setPay(rs.getInt("Pay")); // 村长福利金 此栏位由 HomeTownTimeController 处理
                                         // update
    ...
    public void storeCharacter(final L1PcInstance pc) {
        ...
        try {
            ...
            con = L1DatabaseFactory.getInstance().getConnection();
            pstm = con
                    .prepareStatement("UPDATE characters SET level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,PkCountForElf=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,HellTime=?,Banned=?,Karma=?,LastPk=?,LastPkForElf=?,DeleteTime=? ,RocksPrisonTime=? WHERE objid=?");
            pstm.setInt(++i, pc.getLevel());
            pstm.setInt(++i, pc.getHighLevel());
            pstm.setLong(++i, pc.getExp());
            pstm.setInt(++i, pc.getBaseMaxHp());
